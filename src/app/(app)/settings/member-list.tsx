...
export function MemberList({ members, canManage }: { members: ProjectMember[]; canManage: boolean }) {
  ...
  if (!members.length) {
    return <p className="text-sm text-ink-faint">No teammates yet — invite one below.</p>;
  }
        ...
        <div key={m.id} className="flex items-center gap-3 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-ink-faint">
              {m.accepted_at ? `Joined ${timeAgo(m.accepted_at)}` : "Invite pending — joins on first sign-in"}
            </p>
          </div>
          <Badge tone={m.role === "member" ? "accent" : "neutral"}>{m.role}</Badge>
          {canManage && (
            <button
