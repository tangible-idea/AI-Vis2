...
import { CONTENT_LANGUAGES, industryPhrase } from "../types";
...
export function buildContentMessages(input: BuildInput): ChatMessage[] {
  ...
  const industry = industryPhrase(project.industry);
  ...
  const system = [
    ...
    `Context — brand: "${project.name}", official website: ${project.website} (treat this domain as the canonical identifier for the brand), industry: ${industry}, country: ${project.country}.`,
  ...
  const asks: Record<ContentType, string> = {
    faq_page: `Write a complete FAQ page for ${project.name} with 8–10 questions real buyers ask AI assistants about ${industry} (e.g. "What is the best ${industry} solution?"). Answer each in 2–4 sentences, naming ${project.name} naturally where honest. End with an FAQPage JSON-LD block wrapping the same Q&As.`,
    blog_post: `Write a 900–1200 word authority blog post: "How to choose the right ${industry} solution". Use clear H2/H3 structure, a comparison criteria checklist, and position ${project.name} honestly among the options. Add a TL;DR at the top — AI engines lift these.`,
    ...
    category_page: `Write a "Best ${industry} solutions" category page listing 5–6 options including ${project.name}${competitors.length ? ` and ${competitors.slice(0, 2).join(", ")}` : ""}, each with a 2–3 sentence balanced blurb, followed by a buying guide section.`,
    location_page: `Write a location landing page: "${industry} in ${project.country}" for ${project.name}. Cover local relevance, who it serves in that market, and a local FAQ (4 questions).`,
    schema: `Output JSON-LD structured data for ${project.name}: an Organization block, a Product/Service block for its ${industry} offering, and an FAQPage block with 4 sample Q&As. Output only the <script type="application/ld+json"> blocks, ready to paste.`,
    ...
    internal_links: `Produce an internal linking plan for ${project.website} that concentrates topical authority on ${industry}: list 8–10 concrete link placements (from page → to page, suggested anchor text) and a short rationale for each.`,
    social_post: `Write 3 variants of a short LinkedIn post for ${project.name} about ${industry} — one thought-leadership angle, one practical-tips angle, one trend-reaction angle. Each ≤120 words, no hashtag spam (max 3), written to earn genuine engagement in ${project.country}.`,
